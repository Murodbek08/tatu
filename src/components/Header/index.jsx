...
//             <div className="w-10 h-10  rounded-xl flex items-center justify-center font-black text-white text-[11px] tracking-tight ">
...
function Header() {
  ...
  return (
    <>
      {/* Utility bar */}
      {/* Fon to'q rangga o'zgartirildi */}
      <div className="bg-[var(--bg-dark-section)] border-b border-white/10 text-white/45 text-xs h-10 flex items-center">
        <div className="max-w-7xl mx-auto px-6 w-full flex items-center justify-between">
          <span className="truncate">{t("utilityBar.ministry")}</span>
          <div className="flex p-0.1 bg-black/20 border border-white/10 rounded-full w-fit backdrop-blur-sm">
            {languages.map((lng) => (
              <button
                key={lng}
                onClick={() => i18n.changeLanguage(lng)}
            ...
            ${
              i18n.language === lng
                ? /* Amber ranglari Asosiy Yalpiz (primary) rangiga almashtirildi */
                  "bg-[var(--color-primary)] text-[var(--bg-dark-section)] shadow-lg shadow-[var(--color-primary)]/20"
                : "text-gray-400 hover:text-white"
            }`}
              >
                {t(`lang_${lng}`)}
      ...
      <nav
        className={`sticky top-0 z-[10000] transition-all duration-300 ${
          scrolled
            ? /* Fon to'q rangga o'zgartirildi */
              "bg-[var(--bg-dark-section)]/95 backdrop-blur-md border-b border-white/10 shadow-2xl"
      ...
      >
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <Link to={"/"} className="flex items-center gap-3 group shrink-0">
            ...
            <div className="text-left">
              ...
              {/* Amber rang Yalpiz (primary) ga almashtirildi */}
              <p className="text-[var(--color-primary)] text-[10px] font-medium tracking-wider mt-0.5">
                {t("nav.logoSubtitle")}
              </p>
            </div>
          </Link>
            ...
            {NAV_LINKS.map((link) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `px-3 py-2 rounded-lg text-[13px] font-semibold transition-all border-b-2 ${
                    isActive
                      ? /* Aktiv menyu rangi Yalpiz (primary) ga almashtirildi */
          ...
          <div className="flex items-center gap-3">
            <Link
              to={"admissions"}
              /* Tugma rangi Yalpiz (primary) ga, matni to'q fondagi kabi o'qilishi uchun qora-kulrangga almashtirildi */
              className="hidden sm:block bg-[var(--color-primary)] hover:brightness-95 active:scale-95 text-[var(--bg-dark-section)] text-[13px] font-bold px-5 py-2 rounded-lg transition-all shadow-md shadow-[var(--color-primary)]/30"
            >
              {t("nav.applyButton")}
            </Link>
        ...
        {/* Mobile menu */}
        {open && (
          /* Mobile menyu foni to'q rangga moslashtirildi */
          <div className="xl:hidden bg-[var(--bg-dark-section)] border-t border-white/10 px-6 py-4 space-y-1">
            {NAV_LINKS.map((link) => (
              <NavLink
                key={link.page}
                to={link.page}
                ...
                className={({ isActive }) =>
                  `block w-full text-left px-4 py-3 rounded-xl text-sm font-semibold transition-colors ${
                    isActive
                      ? /* Aktiv fon yalpiz (primary) ga o'zgardi */
            ...
            <Link
              to="/admissions"
              onClick={() => setOpen(false)}
              /* Tugma rangi o'zgartirildi */
              className="block w-full mt-2 text-center bg-[var(--color-primary)] hover:brightness-95 text-[var(--bg-dark-section)] text-sm font-bold py-3 rounded-xl transition-colors"
            >
              {t("nav.applyButton")}
            </Link>
          </div>
