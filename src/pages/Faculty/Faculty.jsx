...
function Faculty() {
  const { t, i18n } = useTranslation(); // i18n obyekti qo'shildi
  const [search, setSearch] = useState("");
  const [selectedStaff, setSelectedStaff] = useState(null);
  const [showContact, setShowContact] = useState(false);
  const [facultyList, setFacultyList] = useState([]); // Dinamik ma'lumotlar uchun state
  const [loading, setLoading] = useState(true);

  // 3 tilli ma'lumotni olish funksiyasi (getField)
  const getField = (obj, fieldName) => {
    const currentLang = i18n.language; // 'uz', 'ru' yoki 'en'
  ...
  useEffect(() => {
    const fetchFaculty = async () => {
      try {
        setLoading(true);
        const { data } = await request.get("/faculty?select=*");
        setFacultyList(data);
      } catch (err) {
        console.error("Xatolik yuklashda:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  // Filtrlash mantiqi (getField orqali)
              ...
              >
                ...
                <div className="flex gap-4 mb-6">
                  <div className="relative shrink-0">
                    <div
                      className={`w-16 h-16 ${m.avatar_cls} rounded-full flex items-center justify-center text-white text-lg font-black overflow-hidden border-2 border-white shadow-sm group-hover:scale-105 transition-transform duration-300`}
                    >
                      {m.image ? (
                        <img
                          src={m.image}
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-[12px] mt-0.5">
                      {t("faculty.deptLabel")} {getField(m, "dept")}
                    </p>
                  </div>
                </div>
            ...
            {selectedStaff && (
              ...
              >
                ...
                >
                  ...
                  <div
                    className={`h-20 ${selectedStaff.avatar_cls} opacity-10 absolute top-0 left-0 w-full pointer-events-none`}
                  />
                  ...
                  <div className="relative p-6 md:p-8 overflow-y-auto">
                    ...
                    <div className="flex flex-col md:flex-row gap-6 items-center md:items-start">
                      <div className="shrink-0 text-center md:text-left">
                        <div
                          className={`w-24 h-24 md:w-28 md:h-28 ${selectedStaff.avatar_cls} rounded-2xl overflow-hidden border-4 border-white shadow-lg mb-3 mx-auto`}
                        >
                          {selectedStaff.image ? (
                            <img
                              src={selectedStaff.image}
                              className="w-full h-full object-cover"
                              alt={getField(selectedStaff, "name")}
                            />
                          ) : (
                    ...
                    <div className="mt-8 space-y-3">
                      <motion.button
                        ...
                        className={`w-full py-3.5 rounded-xl text-white font-bold text-xs shadow-md transition-all ${selectedStaff.avatar_cls}`}
                      >
                        {showContact
